...
The `ggnetwork` package contains the `ggnetwork` ‘workhorse’ function and four geoms that can be used to plot a network as a pure `ggplot2` object. It is meant to be used with [ggplot2](https://github.com/hadley/ggplot2) version 1.0.1.9003 or above. Right now, this means that you need to install `ggplot2` from its development repository to be able to use this package:
...
`ggnetwork` also requires the `network` and `sna` packages for network manipulation, and the `plyr` package for data wrangling.
...
library(network)
library(sna)
n = network(rgraph(10, tprob = 0.25), directed = FALSE)
```
...
And finally, let's now define a blank `ggplot2` theme:
